...
compile_source_with_tag2 emqx/NanoSDK_Mirror.git NanoSDK nanwang-dev "-DNNG_ENABLE_QUIC=OFF -DNNG_ENABLE_TLS=ON -DNNG_TLS_ENGINE=open -DOPENSSL_ROOT_DIR=$install_dir -DNNG_ENABLE_SQLITE=ON -DBUILD_DEMO=OFF -DNNG_ENABLE_SCRAM=ON -DNNG_TESTS=OFF -DBUILD_SHARED_LIBS=OFF"